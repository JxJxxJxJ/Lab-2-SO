...
/*
    ...
    Ya pasa los tests con MAX_SEM = 2
*/
#define MAX_SEM 500
...
/*
    DESCRIPCION: Abre el semáforo “sem” con un valor arbitrario “value”.
    ...
    POSTCONDICION: Si rc = sem_open(sem, value) entonces:
                    Si "sem" estaba cerrado -> rc = SUCCESS_CODE y semaforo sem
   tiene valor = value. Si "sem" estaba abierto -> rc = ERROR_CODE y no modifica
   nada.
*/
...
/*
    ...
    DESCRIPCION: Despierta los procesos pendientes del semaforo "sem".
                 Cierra el semáforo “sem”. Si "sem" ya estaba cerrado
                 no modifica nada y retorna ERROR_CODE.

    PRECONDICION: 0 <= sem <= MAX_SEM.
    POSTCONDICION: Si rc = sem_close(sem) entonces:
                    Si "sem" estaba abierto -> rc = SUCCESS_CODE
                    Si "sem" estaba cerrado -> rc = ERROR_CODE y no modifica
   nada.
*/
uint64 sem_close(int sem);

/*
    DESCRIPCION: Incrementa el semáforo ”sem” desbloqueando los procesos cuando
   su valor i = 0.

    PRECONDICION: 0 <= sem <= MAX_SEM && semaforo "sem" tiene value i.
    POSTCONDICION: semaforo "sem" tiene value i+1
                   && procesos pendientes del semaforo "sem" fueron despertados
   si el i = 0.
*/
uint64 sem_up(int sem);

/*
    DESCRIPCION: Decrementa el semáforo ”sem” bloqueando los procesos cuando su
   valor i = 0.
...
/*
    DESCRIPCION: Busca el primer semáforo inactivo en el arreglo de semáforos
   `sem_array`. Al hallarlo, abre ese semáforo e inicializa su valor en value.

    PRECONDICION: 0 <= value.
    POSTCONDICION: Si i = sem_find_free_channel_and_set(value) entonces
                      Si habia un canal libre    => i     = numero del semaforo
   libre
                                                 && value = valor del semaforo
                      Si no habia un canal libre => i     = CLOSED_SEM_VALUE
*/
